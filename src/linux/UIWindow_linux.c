										   ...
										   struct wl_array *states)
{
	...
	// struct ext_blur_mask_v1 *blurMask = ext_blur_mask_manager_v1_get_blur(UIPlatformGlobalsShared.blur_mask_manager,
	// platformLayer->surface); ext_blur_mask_v1_set_mask(blurMask);

	if (platformWindow->eglWindow != NULL)
	{
		wl_egl_window_resize(platformWindow->eglWindow, requestedSize.size.width, requestedSize.size.height, 0, 0);
	}
	else
	{
	...
	if (eglMakeCurrent(UIPlatformGlobalsShared.eglData.eglDisplay, platformWindow->eglSurface,
					   platformWindow->eglSurface, UIPlatformGlobalsShared.eglData.eglContext) == EGL_FALSE)
	{
...
void _UIPlatformWindowCreate(UIWindow *window)
{
	...
	platformWindow->xdgSurface =
		xdg_wm_base_get_xdg_surface(UIPlatformGlobalsShared.wm_base, platformWindow->wlSurface);
