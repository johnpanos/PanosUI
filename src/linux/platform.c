									...
									uint32_t version)
{
	...
	{
		UIPlatformGlobalsShared.compositor = wl_registry_bind(registry, id, &wl_compositor_interface, 1);
	}
	...
	{
		UIPlatformGlobalsShared.blur_mask_manager =
			wl_registry_bind(registry, id, &ext_blur_mask_manager_v1_interface, 1);
	}
}
