...
static void setup_cursor_image(void *data, uint32_t serial)
{
	...
	wl_surface_attach(seatData->cursor_surface, wl_cursor_image_get_buffer(image), 0, 0);
	wl_surface_damage(seatData->cursor_surface, 1, 0, image->width, image->height);
	wl_surface_commit(seatData->cursor_surface);
	wl_pointer_set_cursor(seatData->wl_pointer, serial, seatData->cursor_surface, image->hotspot_x, image->hotspot_y);
}
...
static void wl_pointer_motion(void *data, struct wl_pointer *wl_pointer, uint32_t time, wl_fixed_t surface_x,
							  wl_fixed_t surface_y)
{
	...
	UIEvent event = {.type = UIEventTypeMouseMotion,
					 .window = seatData->window,
					 ._eventData = {.mouseMotion = {.x = wl_fixed_to_int(surface_x), .y = wl_fixed_to_int(surface_y)}}};

	UIApplicationSendEvent(event);
}

static void wl_pointer_button(void *data, struct wl_pointer *wl_pointer, uint32_t serial, uint32_t time,
							  uint32_t button, uint32_t state)
{
	struct UIPlatformSeatData *seatData = (struct UIPlatformSeatData *)data;

	UIEvent event = {.reserved = serial,
					 .type = state ? UIEventTypeMouseDown : UIEventTypeMouseUp,
					 .window = seatData->window,
					 ._eventData = {.mouseButton = {.button = button == BTN_LEFT ? UIEventMouseButtonTypeLeft
																				 : UIEventMouseButtonTypeRight,
													.state = state}}};
...
static void wl_pointer_axis(void *data, struct wl_pointer *wl_pointer, uint32_t time, uint32_t axis, wl_fixed_t value)
{
	...
	UIEvent event = {.type = UIEventTypeMouseScroll,
					 .window = seatData->window,
					 ._eventData = {.mouseScroll = {.direction = axis, .type = UIEventMouseScrollTypeContinuous}}};
